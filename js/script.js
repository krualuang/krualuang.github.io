...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function setLanguage(lang) {
        ...
        document.querySelectorAll('[data-i18n]').forEach(element => {
            ...
            if (currentTranslations[key]) {
                // Check if it's an input value or just text content
                if (element.tagName === 'INPUT' || element.tagName === 'BUTTON' || element.tagName === 'TEXTAREA') {
                     element.value = currentTranslations[key];
                } else if (element.tagName === 'A' && element.textContent !== '') {
                    // This handles navigation links which have both data-i18n and textContent
        ...
        // Special handling for the form submit button text content
        const submitButton = document.querySelector('.submit-btn');
        if(submitButton && currentTranslations['contact.form.submit']) {
            submitButton.textContent = currentTranslations['contact.form.submit'];
        }
        
        // Custom update for the specific hero title placeholder in the minimal HTML structure
        const homeTitleElement = document.querySelector('[data-i18n="home.title"]');
        const homeSubtitleElement = document.querySelector('[data-i18n="home.subtitle"]');
        if (homeTitleElement && currentTranslations['hero.title']) {
            // Since the minimal HTML uses 'home.title' and 'home.subtitle' for the hero section,
            // we map them to the corresponding full text keys.
            homeTitleElement.textContent = currentTranslations['hero.title'];
        }
        if (homeSubtitleElement && currentTranslations['hero.desc']) {
            homeSubtitleElement.textContent = currentTranslations['hero.desc'];
        }
        
        // Update the HTML language attribute for better screen reader/browser support
    ...
    window.addEventListener('scroll', () => {
        ...
        // Adjust 500 based on where you want the button to appear (in pixels)
    ...
    if (savedLang && translations[savedLang]) {
        ...
        setLanguage('th'); // Defaulting to Thai as per original HTML
    }
});
